...
Whenever a node produces a block, that node may collect the difference between the amount of routing work required by consensus for block production and the actual amount of routing work included in the mempool for that block i.e. block producers are entitled to any fees which make up work done in excess of what was required to make a valid block. This is the only payment made for block production, and despite being a minuscule proportion compared to the other potential payments made, block producers' only other option is to allow those reward go to a competing node.

Unlocking the greater remainder of these payments requires the network to solve a computational puzzle, the "golden ticket". This puzzle requires knowledge of the block hash to solve and cannot be calculated in advance. Miners on the network listen for blocks as they are produced and begin hashing in search of a solution. Should they find a solution it is propagated into the network as a normal fee-paying transaction.

Only one solution may be included in any block, and the solution must solve the previous block for the current block to be considered valid. If these conditions are violated or a golden ticket is not solved, the funds that were not paid out in the previous block are simply not allocated. They fall backwards into the blockchain and, reaching their epoch's end, are recollected by the consensus layer and redistributed as part of a future block reward.
